## EventBus autoload'un doğru çalıştığını test eder.
## Bu aynı zamanda test altyapısının (GdUnit4) çalıştığını kanıtlar.
...
func test_EventBus_EmitSignal_MoneyEarned_ShouldBeReceived() -> void:
	# Dictionary referans tipi — closure içinden değiştirilebilir
